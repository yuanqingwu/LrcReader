 ...
 */
public class SongEntityListAdapter extends BaseRecyclerViewAdapter<SongEntityListAdapter.SongListViewHolder> {
    private Context context;
    private List<? extends ISong> list;

    public SongEntityListAdapter(Context context, List<? extends ISong> list) {
        this.context = context;
        this.list = list;
    }

    public List<? extends ISong> getDataList() {
        return list;
    }

    public void refreshData(List<? extends ISong> newList) {
        if (list == null) {
            list = newList;
...
//        holder.albumText.setText(list.get(position).getAlbum());
        ...
        String coverUri = list.get(position).getAlbumCover();
        LogUtil.i("coverUri:" + coverUri);
        //fixme 如果是网址则加载，如果是本地图片则直接显示
        if (coverUri != null) {
            if (coverUri.startsWith("http")) {
                coverUri = list.get(position).getAlbumCover().replace("/cover/", "/album-cover/");
            }
